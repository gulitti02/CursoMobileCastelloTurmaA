...
class _TarefasViewState extends State<TarefasView> {
  ...
  final _db = FirebaseFirestore
      .instance; //controller para enviar tarefas para o BS FireStore
  final User? _user =
      FirebaseAuth.instance.currentUser; //pega o usuário logado atualmente
          ...
          children: [
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                //dependendo do resultado da Query exibe a tela desejada
                stream: _db
                    .collection("usuarios")
                    .doc(_user?.uid)
                    .collection("tarefas")
                ...
                builder: (context, snapshot) {
                  ...
                  //exibir as tarefas
                  final tarefas =
                      snapshot.data!.docs; //adicona a busca ao vetor de tarefas
                  return ListView.builder(
                    itemCount:
                        tarefas.length, //conta o numero de tarefas no vetor
                    itemBuilder: (context, index) {
                      ...
                      final tarefaMap =
                          tarefa.data()
                              as Map<
                                String,
                                dynamic
                              >; //converte a tarefa para um mapa
                      //ajusta da variavel booleeana
                      ...
                      return ListTile(
                        title: Text(tarefaMap["titulo"] ?? "Sem Título"),
                        leading: Checkbox(
                          value: concluida,
                          onChanged: (value) {
                            setState(() {
